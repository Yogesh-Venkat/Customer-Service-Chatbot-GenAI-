...
def build_prompt(query: str, contexts: List[str]) -> str:
    ctx = "\n\n---\n\n".join(contexts)
    prompt = f"""
You are a helpful, accurate customer support assistant. Answer the user's question using ONLY the provided context.
If the answer is not present in the context, say: "I'm not sure about that from the current knowledge base."
...
Use bullet points if it helps. Keep the tone professional and friendly.
"""
    return prompt.strip()
...
def generate_response(model, user_query: str, contexts: List[str]) -> Tuple[str, float]:
    ...
    if not text:
        text = "Sorry — I could not generate an answer."
    return text, latency
...
def main():
    ...
    with col2:
        k = st.slider("📚 Sources to retrieve", min_value=1, max_value=10, value=TOP_K)
